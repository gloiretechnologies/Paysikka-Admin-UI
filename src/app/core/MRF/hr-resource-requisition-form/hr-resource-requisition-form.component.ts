...
@Component({
  selector: 'app-hr-resource-requisition-form',
  templateUrl: './hr-resource-requisition-form.component.html',
  styleUrls: ['./hr-resource-requisition-form.component.scss']
})
export class HrResourceRequisitionFormComponent {
  ...
  constructor(private formBuilder: FormBuilder, private api:ResourceRequisitionService, private toast: NgToastService) {
    this.form = this.formBuilder.group({
      ...
      leaving_ename:new FormControl('',Validators.required),
      date_leaving:new FormControl('',Validators.required),
      dept_strenght:new FormControl('',Validators.required),
      strenght_function:new FormControl('',Validators.required),
  ...
  onSubmit(): void {
    ...
    if (this.form.valid) {
      ...
      this.api.updateresourceByHR(data).subscribe((res:any)=>{
      console.log('res',res)
        this.showSuccess();
  ...
  editorText:any;
  myGetdata(id:any){
    localStorage.setItem('hrfrmId',id)
    this.api.getSingleResource(id).subscribe((res:any)=>{
      res;
      ...
      console.log('format date',formatApprovedDate)
